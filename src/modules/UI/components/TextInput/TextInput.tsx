...
export function TextInput({
  ...
  return (
    <View className={`gap-1.5 ${containerClassName}`}>
      {label ? (
        <Text className="text-footnote font-medium uppercase tracking-wide text-label-secondary">{label}</Text>
      ) : null}
      <RNTextInput
        editable={editable}
        ...
        {...props}
      />
      {error ? <Text className="text-footnote text-destructive">{error}</Text> : null}
    </View>
  );
}
